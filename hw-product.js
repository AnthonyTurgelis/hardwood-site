...
(function(){
  ...
  function init(){installShell();var page=document.body.dataset.page||"";if(page==="home"&&$("home-kpis"))Promise.all([fetchJSON("games.json"),fetchJSON("preview.json"),fetchJSON("players.json"),fetchJSON("teams.json"),fetchJSON("standings.json"),fetchJSON("accuracy.json"),fetchJSON("deepdive_index.json")]).then(function(v){renderHome({games:v[0],preview:v[1],players:v[2],teams:v[3],standings:v[4],accuracy:v[5],dives:v[6]});});else if(page==="games")initGames();else if(page==="players")initPlayers();else if(page==="teams")initTeams();else if(page==="standings")initStandings();else if(page==="accuracy")initAccuracy();else if(page==="research")initResearch();}
  window.HWProductShell={install:installShell};
